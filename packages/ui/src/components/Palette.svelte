...
<script lang="ts">
  ...
  function recents(): string[] {
    try { return JSON.parse(storage.getItem("ss:recent") || "[]"); } catch { return []; }
  }
  function remember(id: string) {
    const r = [id, ...recents().filter((x) => x !== id)].slice(0, 8);
    try { storage.setItem("ss:recent", JSON.stringify(r)); } catch {}
  }
  ...
  $effect(() => { const timer = setTimeout(() => input?.focus(), 10); return () => clearTimeout(timer); });
  $effect(() => {
    const el = app.root?.querySelector<HTMLElement>(`.pal li.on`);
