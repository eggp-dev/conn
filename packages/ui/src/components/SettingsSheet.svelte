<script lang="ts">
  ...
  const { selectTheme, invoke, st, cur, tab, toast, cmd, changeMode, liveAgents, watchLiveAgents, THEMES, t, i18n, setLang, storage } = app;
  ...
  import { type Analysis } from '../lib/store.svelte';
  import { TOOLS, TOOL_PRESETS, type Mode } from '../lib/bridge';
  ...
  async function copyAgentConfiguration() {
    ...
    try {
      ...
      try {
        await app.copyText(config[configFormat]);
...
<details class="advanced"><summary>{t("s.advancedPermissions")}</summary>
      ...
      {:else if st.settingsTab === "appearance"}
        ...
        <label class="row"><input type="checkbox" checked={st.followSystem} onchange={(e) => { st.followSystem = (e.target as HTMLInputElement).checked; storage.setItem("ss:followSystem", st.followSystem ? "1" : "0"); }} /> {t("s.follow")} <em class="muted">{t("s.follow.hint")}</em></label>
        <label class="row"><input type="checkbox" checked={st.effects} onchange={(e) => { st.effects = (e.target as HTMLInputElement).checked; storage.setItem("ss:effects", st.effects ? "1" : "0"); }} /> {t("s.effects")}</label>
        <h3>{t("s.font")} <span class="v">{st.fontSize}px</span></h3>
        <input aria-label={t("s.font")} type="range" min="10" max="20" step="1" value={st.fontSize} onchange={(e) => { st.fontSize = +(e.target as HTMLInputElement).value; storage.setItem("ss:fontSize", String(st.fontSize)); }} />
