<script lang="ts">
  ...
  async function copy(it: TimelineItem) {
    try { await app.copyText(it.text); toast(t("copied"), "ok"); }
  ...
  async function retype(it: TimelineItem) {
    ...
    (app.root?.querySelector('.host:not([hidden]) .xterm-helper-textarea') as HTMLElement | null)?.focus();
  }
