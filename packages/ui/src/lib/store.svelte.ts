...
export function createStore(storage: NonNullable<ConnPorts["storage"]>, lifetime: Lifetime) {
  const st = $state({
    ...
    themeRevision: 0,
    theme: (storage.getItem("ss:theme") as ThemeId) || "midnight",
    followSystem: storage.getItem("ss:followSystem") === "1",
    effects: storage.getItem("ss:effects") !== "0",
    fontSize: Number(storage.getItem("ss:fontSize") || 13),
    paletteOpen: false,
    centerOpen: false,
    sharingOpen: false,
  ...
  function cur(): TabState {
    return st.tabs[st.active] ?? placeholder;
  }
  function tab(id: string): TabState | undefined {
    return st.tabs[id];
  }
  ...
  function toast(text: string, cls = "") {
    ...
    lifetime.timeout(() => { st.toasts = st.toasts.filter((t) => t.id !== id); }, 4000);
  }

  let annSeq = 0;
  let annTimer: number | null = null;
  function announce(text: string, color: string, kind: Announcement["kind"] = "info", ms = 2600, detail?: string, target?: NoticeTarget) {
    const id = ++annSeq;
    st.announcement = { id, text, detail: detail?.trim() || undefined, ms, color, kind, target };
    if (annTimer) lifetime.clearTimeout(annTimer);
    annTimer = lifetime.timeout(() => { if (st.announcement?.id === id) st.announcement = null; }, ms);
  }


  return { st, cur, tab, tabIndex, toast, announce };
}
