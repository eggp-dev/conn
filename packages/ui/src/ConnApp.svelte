<script lang="ts">
  ...
  import { createConnApp } from './runtime/app.svelte';
  import { provideConnApp } from './runtime/context';
  let { ports }: { ports: ConnPorts } = $props();
  // Host ports are immutable for this mount; replacing a host requires unmounting it.
  // svelte-ignore state_referenced_locally
  const app = provideConnApp(createConnApp(ports));
  onDestroy(app.dispose);
  const { signalPending, admissionSnapshot, resolveReview, resolveControl, refreshExtensions, selectTheme, refreshProfiles, invoke, listen, cmd, changeMode, onEvent, onTabOpened, onAdmission, log, st, cur, tab, tabIndex, toast, announce, THEMES, t: tr, tabName, fmtMs, setLang, i18n, storage } = app;
  ...
  import { TOOL_PRESETS } from './lib/bridge';
  import { newTab, type TabState } from './lib/store.svelte';
  import { agentColor } from './lib/themes';
  import { LANGS } from './lib/i18n.svelte';
  ...
  const focusTerm = () => app.timeout(() => terms[st.active]?.focus(), 0);

  function wash(t: TabState, color: string, out: boolean) {
    t.wash = { ...(terms[t.id]?.cursorCenter() ?? { x: 4, y: 8 }), color, out, key: ++washSeq };
    app.timeout(() => { if (t.wash?.key === washSeq) t.wash = null; }, 1300);
  }
  function showHandback(t: TabState) {
    if (!t.lastAgent) return;
    t.handback = true;
    if (handbackTimers[t.id]) app.clearTimeout(handbackTimers[t.id]);
    handbackTimers[t.id] = app.timeout(() => (t.handback = false), 60_000);
  }
  ...
  async function focusRequest(key:string) {
    await tick();
    [...(app.root?.querySelectorAll<HTMLElement>('[data-request-key]') ?? [])].find(el=>el.dataset.requestKey===key)?.focus();
  }
  ...
  const actions = $derived.by((): Action[] => {
    ...
    const all: Action[] = [
      ...
      { id: "effects", group: "theme", label: tr("a.effects"), aliases: ["effects", "효과", "wash"], value: () => (st.effects ? tr("on") : tr("off")), run: () => { st.effects = !st.effects; storage.setItem("ss:effects", st.effects ? "1" : "0"); } },
  ...
  const parsers: Parser[] = [
    ...
    (q) => { const m = num(q, /^(?:font|글꼴|size)\s*(\d+)\s*(px)?$/i); if (!m) return []; return [{ id: "set-font", group: "theme", label: tr("a.font.set", { v: m.n }), run: () => { st.fontSize = m.n; storage.setItem("ss:fontSize", String(m.n)); } }]; },
  ...
  function onKey(e: KeyboardEvent) {
    if (e.target instanceof Node && app.root && !app.root.contains(e.target) && e.target !== document.body) return;
    if (!appShortcut(e)) {
  ...
  onMount(() => {
    ...
    async function restoreView(restarted: boolean) {
      const revision = ++restoreRevision;
      if (restarted) {
        for (const id of st.order) app.forgetSession(id);
        st.tabs = {}; st.order = []; st.active = ''; st.tabSeq = 0;
        st.admissions = []; st.activity = { revision: -1, connections: [] };
        st.announcement = null; st.centerOpen = false; st.sharingOpen = false;
        sessions.reset(); activity.reset();
        await tick();
      }
      try {
        ...
        st.socket = info.socket ?? ''; st.externalPending = !!info.externalPending;
        for (const id of [...st.order]) if (!info.sessions.includes(id)) { app.forgetSession(id); delete st.tabs[id]; }
        st.order = st.order.filter(id => info.sessions.includes(id));
        for (const id of info.sessions) if (!tab(id)) addTab(id);
        if (!tab(st.active)) st.active = info.session ?? st.order[0] ?? '';
        await Promise.all(info.sessions.map(syncStatus));
        await reconcileAdmissions(); await refreshActivity();
        if (!disposed && revision === restoreRevision) {
          booted = true; startupFailed = false; st.backendOnline = true; await tick(); await invoke('ui_ready');
          if (!statusTimer) statusTimer = setInterval(() => { for (const id of st.order) void syncStatus(id).catch(() => {}); }, 5000);
          app.timeout(() => { terms[st.active]?.refit(); }, 50);
        }
      } catch (error) { if (!disposed) { st.backendOnline = false; toast(String(error), 'warn'); } }
    }
    const connectionListener = listen<{ connected: boolean; runtimeId?: string }>("ss:connection", ({payload}) => {
      const restarted = !!runtimeId && !!payload.runtimeId && runtimeId !== payload.runtimeId;
      if (payload.runtimeId) runtimeId = payload.runtimeId;
      st.backendOnline = payload.connected;
      if (!payload.connected) { restoreRevision++; return; }
      admissions = new Admissions();
      if (booted || startupFailed) void restoreView(restarted);
    });
    ...
    const eventListener = onEvent((ev) => {
      ...
      switch (ev.event) {
        ...
        case "agent_switched_tab":
          ...
          break;
        case "agent_input":
          t.typing = true; if (typingTimers[t.id]) app.clearTimeout(typingTimers[t.id]); typingTimers[t.id] = app.timeout(() => (t.typing = false), 500);
          break;
    ...
    void (async () => { try {
      ...
      booted = true; st.backendOnline = true;
      await tick();
      await invoke("ui_ready");
      app.timeout(() => { terms[st.active]?.refit(); focusTerm(); }, 50);
    ...
    return () => {
      disposed = true; for (const timer of [...Object.values(handbackTimers), ...Object.values(typingTimers)]) app.clearTimeout(timer); sessions.dispose(); activity.dispose(); clearInterval(activityTimer); clearInterval(admissionTimer); clearInterval(statusTimer);
