...
3. **요청을 하나 보내세요.** 오른쪽 위 제어 메뉴에서 **Autopilot**, **승인 후 제어권 부여**, **실행 유예 2초**를 설정하고 에이전트에게 요청합니다.
...
Conn은 데스크톱 앱으로도, 기존 터미널 안의 세션으로도 사용할 수 있습니다. 에이전트는 **MCP 또는 CLI**로 연결하며, Conn 자체가 모델을 실행하지는 않습니다. 영상에서는 Codex CLI를 사용했습니다. 다른 MCP 클라이언트에는 복사한 서버 설정을 등록하면 됩니다.
