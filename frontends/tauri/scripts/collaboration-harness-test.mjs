...
const port=process.env.CONN_HARNESS_UI_PORT??'1437', backend=process.env.CONN_HARNESS_PORT??'1438';
...
const {run} = await collaborationBrowser();
...
try {
  ...
  await run('open',`http://127.0.0.1:${port}/`);await run('wait','.xterm-helper-textarea');
  await run('click','.xterm-helper-textarea');await run('type','.xterm-helper-textarea','export CONN_COLLAB_PROCESS=retained_42');await run('press','Enter');await delay(350);
  await run('wait','button.dot:not(.hidden)');await run('click','button.dot:not(.hidden)');await run('wait','.center');await run('click','.sharing-row button');await run('click','.sharing footer .ghost');await run('wait','.sharing-trigger');
  ...
  await run('wait','.decision-card');await run('screenshot',resolve(output,'harness-private-admission.png'));await run('click','.decision-card .primary');await delay(300);await run('screenshot',resolve(output,'harness-after-allow.png'));
  const hidden=await call('list_tabs');assert.equal(hidden.tabs,0);
  await run('click','.sharing-trigger');await run('click','.sharing-row button');await run('click','.sharing input[type=checkbox]');await run('click','.sharing footer .btn:not(.ghost)');
  ...
  await run('wait','.decision-card');await run('click','.decision-card .primary');await control;
  ...
  if(execution.status === 'pending') { await run('wait','.decision-card .btn.ok');await run('click','.decision-card .btn.ok'); }
  ...
  await run('screenshot',resolve(output,'harness-shared-execution.png'));
  await run('wait','button.dot:not(.hidden)');await run('click','button.dot:not(.hidden)');await run('wait','.center');await run('click','.sharing-row button');await run('click','.sharing footer .ghost');
  ...
  try{await run('close');}catch{}vite?.kill();server.kill();
}
