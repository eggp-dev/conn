    ...
    return new Promise<WebSocket>((resolve,reject)=>{
      ...
      ws.onmessage=({data})=>{
        ...
        if(v.event==="ss:output"){
          const id=v.payload.session;const chunks=history.get(id)??[];chunks.push(v.payload.data);history.set(id,chunks);
          let count=(historyBytes.get(id)??0)+v.payload.data.length;
          while(count>512*1024 && chunks.length>1)count-=chunks.shift()!.length;
...
export async function listen<T>(name:string,handler:(event:{payload:T})=>void):Promise<()=>void>{
  ...
  if(name==="ss:output")for(const [session,chunks]of history)for(const data of chunks)handler({payload:{session,data} as T});
