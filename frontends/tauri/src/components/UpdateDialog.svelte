<script lang="ts">
  ...
  import { updater, initUpdates, updateAction, updatePreference } from '../lib/nativeUpdates.svelte';
  ...
  async function open() { try { await invoke('open_release', { url: s.version ? `${RELEASES}tag/v${s.version}` : RELEASES }); } catch { openFailed = true; } }
...
<dialog bind:this={dialog} class="palette-surface" aria-labelledby="update-title" onkeydown={(e) => e.stopPropagation()} onclose={onclose} oncancel={onclose}>
  ...
  <footer>
    ...
    {:else}
      <button onclick={() => updateAction('check')} disabled={updater.busy || s.phase === 'ready'}>{t('update.retry')}</button>
