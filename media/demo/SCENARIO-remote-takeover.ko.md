...
상태: **촬영 완료** · 2026-09-19. 결과물과 출처는 [RECORDING-REMOTE.md](RECORDING-REMOTE.md)에 있습니다. 기획과 달라진 점: 컷 편집 없이 한 테이크로 찍었고, `sudo` 재시작 장면은 뺐으며(원격에서는 모든 카드가 같은 검토 라벨이라 의미가 겹침), 에이전트 클라이언트 화면 없이 Conn 창 하나만 보여 줍니다. 기존 제작 방식은 [README](README.md)와 [HANDOFF](HANDOFF.md)를 따릅니다.
