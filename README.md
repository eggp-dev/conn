...
Mac downloads are signed and notarized. Windows previews are unsigned. Linux builds use Ubuntu 24.04; Intel Mac distribution is paused. [Installation and updates](docs/getting-started.md) · [All assets and checksums](https://github.com/eggplantiny/conn/releases/tag/v0.8.0)
...
v0.8.0 lets agents keep working in a shared session while its window is in the background, asks before a new agent joins, and keeps control to one tab at a time. v0.7.0 added sharing within the same running shell, themes and opt-in native command
