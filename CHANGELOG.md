...
## Unreleased — Shared surface hard cut
...
This is a breaking development change, not a published version. Restart the app and its MCP clients together; live sessions are not migrated. Existing profile/settings files and saved activity remain. Reconnected agents do not inherit explicit selection by display name. See [the product contract](docs/PRD.md), [protocol v2](docs/protocol.md) and [extensions](docs/extensions.md).

한국어: 실제 표시 화면을 관찰 기준으로 통합하고, 같은 셸에서 연결별 공유를 시작·중단할 수 있게 했습니다. 구형 headless·Entrust·공개 소켓의 소유자 경로를 제거하고, 선언형 테마와 OS 키 저장소 기반 자동완성을 추가합니다. 미배포 변경이며 앱과 MCP를 함께 재시작해야 합니다. 기존 파일과 저장 기록은 유지하지만 실행 중인 셸을 이전하지는 않습니다.
