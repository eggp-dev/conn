...
추후 Windows·Linux 어댑터도 같은 내부 계약을 사용합니다.
...
현재 `Session::track_human`은 Enter까지의 원시 입력을 명령으로 취급합니다.
외부 출처 세션에서는 첫 바이트 이전부터 이 추적을 끄고, 나중에 공유해도 다시 켜지 않습니다.
사람 입력은 병렬 원문 복사본을 만들지 않고 PTY로 전달합니다.

일반 세션에도 별도의 관련 결함이 있습니다. 이번 자동화 수정만으로 전체 앱의 암호
입력 보호를 완료했다고 설명해서는 안 됩니다. 일반 세션 수정은 원시 키 입력을 명령
기록의 근거로 쓰지 않도록 바꾸고, 에이전트가 명시적으로 제출한 명령 기록은 유지합니다.
사람의 명령 기록 복원은 신뢰 범위를 설명한 명시적 셸 명령 경계 연동을 통해 설계해야 합니다.
현재 소스에는 해당 연동이 없습니다. 알 수 없는 입력은 저장하지 않는 것이 원칙이며,
사람 타임라인의 기록 범위가 달라지는 별도 작업으로 처리합니다.
...
| F: 일반 입력 추적 | 일반 세션의 무조건적인 입력 기록 별도 수정 | SSH·tmux·raw 모드·암호 입력 재현, 타임라인 범위 결정 |
...
5. macOS에서 실제 Apple Event 전달을 검사합니다. 예제 컴파일과 Linux 테스트만으로
   네이티브 검증을 대체하지 않습니다. Windows·Linux 외부 어댑터는 아직 구현 대상이 아닙니다.
