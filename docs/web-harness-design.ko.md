...
**구현 상태:** 로컬 공통 앱·실행 경계와 독립 웹 패키지를 하드컷으로 구현했습니다(미배포). 아래는 구현의 기준이 된 설계이며, 현재 실행 방법과 검증 범위는 [웹 호스트 안내](browser-testing.ko.md)를 따릅니다. 후속 [macOS 네이티브 검증 결과](shared-runtime-macos-validation-results.ko.md)를 별도로 기록했습니다. 원격 운영은 후속 범위입니다.
