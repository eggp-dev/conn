...
Conn provides its MIT-licensed source and native binaries through [GitHub Releases](https://github.com/eggplantiny/conn/releases/tag/v0.8.1). All desktop packages use the same Rust engine and Svelte UI. The browser harness is a development adapter.

## Targets for upcoming preview releases

Intel Mac distribution is paused after v0.4.1. Previously published Intel assets remain available; this change does not delete or replace them.
...
The Apple Silicon target produces a public `-signing.json` report bound to the final asset hashes. A successful signing report demonstrates those checks, not a completed first-run GUI test. Verify a fresh browser download on Apple Silicon before claiming that interaction is tested.
...
- [ ] Verify bash on Ubuntu, PowerShell and cmd.exe on Windows, and zsh on Apple Silicon.
...
- [ ] macOS: inspect the signing report and test fresh downloaded app/CLI execution on Apple Silicon.
