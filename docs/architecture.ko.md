...
**공유 세션 화면 · 미배포 리팩터링.** [제품 계약](PRD.ko.md) · [English](architecture.md)

공통 UI와 로컬 웹 호스트를 워크스페이스 패키지로 구현했습니다.
[경계 설계](web-harness-design.ko.md)와 [운영·검증 안내](browser-testing.ko.md)를 함께 참고하세요.

```mermaid
flowchart LR
  H[사람] --> U[공통 ConnApp UI]
...
- Tauri와, 쿠키·Origin으로 보호되는 웹 호스트는 같은 `conn-frontend::AppRuntime`를 씁니다.
...
`packages/ui`는 앱별 Svelte 룬 상태, 액션, 구독, 타이머와 터미널 렌더러를 소유합니다. 각 ConnApp은 호스트 포트를 받으며 네이티브·웹 진입점에는 별도 승인 UI가 없습니다. `crates/frontend::AppRuntime`은 명령 처리, 세션별 입력·크기 변경 순서와 화면 연결 epoch를 소유합니다. 생성한 TypeScript DTO가 Rust 소유자 전송 계약을 따릅니다.

웹 서버의 프로세스 수명은 WebSocket과 독립적입니다. 새로고침은 같은 셸에 새 화면을 연결하고, 소유자 전용 체크포인트 뒤에 순번이 있는 출력을 이어받습니다. 같은 셸 묶음의 입력과 크기는 활성 소유자 화면 하나가 결정하며 명시적 인계는 이전 화면을 차단합니다. 과거 입력을 자동 재전송하지 않습니다. 출력 누락 시 재동기화까지 입력을 멈춥니다. 에이전트 스냅샷은 허용된 현재 그리드이며 렌더러 체크포인트를 에이전트 API로 제공하지 않습니다.

현재 패키지는 루프백만 지원합니다. 원격 배포·인증은 후속 범위이며 별도로 실행한 데스크톱 앱을 미러링하지 않습니다. 네이티브 알림과 플랫폼 업데이트는 호스트 기능으로 남습니다.
